...
//        var_dump($result);exit();
    ...
    {
//        var_dump(func_get_args());exit();//array(3) { [0]=> string(9) "tocartest" [1]=> string(6) "b.file" [2]=> string(7) "hi, oss" }
        $this->precheckCommon($bucket, $object, $options);
//        echo 111;exit();
...
//        var_dump($result->getData());exit;
    ...
    public function multiuploadFile($bucket, $object, $file, $options = null)
    {
        ...
        $is_check_md5 = $this->isCheckMD5($options);//true;
        ...
        foreach ($pieces as $i => $piece) {
            ...
            $response_upload_part[] = $this->uploadPart($bucket, $object, $uploadId, $up_options);  // 在uploadPart这里是有问题的
        }
...
//        var_dump($signableQueryStringParams);exit;
    ...
    //20160104 修改headers中的时间戳+8 使gmtdate 产生的时间戳与本地一致.
    //20160104 调试发现服务器使用GMT+0时间,去掉strtotime的+8
